...
const SubmissionsPage = () => {
  ...
  const loadSubmissions = async (page = currentPage) => {
    try {
      const { data } = await fetchAllSubmissions(page, LIMIT);
  ...
  useEffect(() => { loadSubmissions(currentPage); }, [currentPage]);
    ...
    <div className="flex min-h-screen bg-bg-dark">
      ...
      <main className="ml-60 flex-1 px-10 py-9">
        ...
        <div className="grid grid-cols-4 gap-4 mb-7">
          {[
            { label: 'Total',    value: pagination.total, color: 'text-text-primary' },
            ...
            <div className="flex items-center justify-between px-6 py-4 border-t border-border bg-bg-surface">
              ...
              <div className="flex items-center gap-1.5">
                <button
                  ...
                  className="px-3 py-1.5 text-[12px] font-medium border border-border rounded-md text-text-muted hover:text-text-primary hover:bg-bg-hover disabled:opacity-50 disabled:cursor-not-allowed transition-colors">
                  ← Prev
                </button>
                <button
                  onClick={() => setCurrentPage(currentPage + 1)}
