...
const TalentDashboard = () => {
  ...
  const isTasksView = location.pathname === '/talent/tasks';
  ...
  useEffect(() => { loadAvailable(); loadMyTasks(); }, [location.pathname]);
      ...
      <main className="ml-[220px] flex-1 px-8 py-8" style={{ maxWidth: 'calc(100vw - 220px)' }}>
        ...
        <div className="mb-7 page-section">
          <h1 className="text-[22px] font-semibold tracking-tight"
            ...
            {isTasksView ? 'My Tasks' : `Welcome back, ${user?.name?.split(' ')[0]}`}
          </h1>
          <p className="mt-0.5 text-[13px]" style={{ color: '#6B7280' }}>
            {isTasksView 
              ? 'View details, manage status, and submit files for your claimed tasks.' 
